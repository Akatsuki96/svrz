...
class LSVRZD(GVR_ZD):
    ...
    def optimize(self, x0, T, p, gamma : float, h: Callable, option='random', verbose=False):
        x_tau = x0.clone()
        f_values, it_times = [], []
        recompute = True
        for tau in range(T):
            ...
            if torch.rand(1, generator=self.generator) <= p:
                x_tau = x_k
                recompute = True
            else:
                recompute = False
            ...
            if verbose:
                print("[L-SVR-ZD] k = {}/{}\tf(x_k) = {}".format(tau, T, f_values[-1]))
